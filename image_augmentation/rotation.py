import imgaug as ia # 이미지 증강 lubrary
...
# 경로 확인
path = '/content/drive/My Drive/before_aug/night_front/images/train/'
...
for file in files: # 해당 경로에 있는 모든 파일에 대해
  ...
  f = open('/content/drive/My Drive/before_aug/night_front/labels/train/'+labelfile) # 사진에 맞는 라벨 파일 열기
  ...
  seq = iaa.Sequential([iaa.Affine(rotate = 5)]) # 이미지에 20도 회전을 적용하는 Affine 변환을 정의한 시퀀스(증강 파이프라인)를 생성
  ...
  cv2.imwrite('/content/drive/My Drive/after_rotation/night_front/images/train/' + "rotate__" + file, image_aug) # 회전된 이미지 저장
  ...
  f = open('/content/drive/My Drive/after_rotation/night_front/labels/train/' + "rotate__" + labelfile, "w") # 회전 정보를 담은 label파일 저장
